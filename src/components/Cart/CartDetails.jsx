...
export const CartDetails = ({
	...
	const handlePayment = async (pay_type = "") => {
		...
		const shipping_address = localStorage.getItem("shipping_address");
		const shiprocket_charges = localStorage.getItem("shiprocket_charges")
			? JSON.parse(localStorage.getItem("shiprocket_charges"))
			: [];
			...
			if (pay_type == "cod") {
				...
				axios({
					...
					data: {
						...
						shiprocket_charges: shiprocket_charges,
	...
	async function fetchCartProducts() {
		...
		await axios({
			...
			data: {
				customer_id: customer?.id,
				selected_shipping: cartValues?.selected_shipping_fees,
	...
	return (
		<Fragment>
				...
				<div className="card-body">
					<ul className="list-group mb-3">
						<li className="list-group-item d-flex justify-content-between">
							...
							<span className="text-dark fw-bold">
								₹{cart_total.product_tax_exclusive_total}
							</span>
						</li>
						<li className="list-group-item d-flex justify-content-between">
							<b>Taxes</b>
						...
						{coupon?.value?.coupon_code && (
							<li className="list-group-item d-flex justify-content-between">
								<b>
									Coupon [{coupon?.value?.coupon_code}{" "}
									{coupon?.value?.coupon_info?.coupon_type?.discount_type ==
									"percentage"
										? coupon?.value?.coupon_info?.coupon_type.discount_value +
										  "%"
										: ""}
									] (-)
								</b>
							...
							<li className="list-group-item">
								...
								<div className="text-secondary fw-bold">
									<span>
										...
										<b>{selectShippingAddress.post_code}</b>
									</span>
								</div>
							</li>
						</ul>
					...
					<div className="input-group mb-3">
						<input
							...
							value={coupon?.value?.coupon_code}
							role={`${coupon?.value?.coupon_code ? "button" : ""}`}
					...
					<Button
						className="btn-dark text-white w-100"
						size="lg"
					...
					<Button
						className="btn-success text-white w-100 mt-3"
						size="lg"
			...
			<CaptchaForm
				...
				setCodFormloading={setCodFormloading}
