...
export const Layout = () => {
  ...
  const topMenuAll = localStorage.getItem("topMenu")
    ...
    : [];
  const menuAll = localStorage.getItem("allMenu")
  ...
  const getSubMenu = (category) => {
    ...
    const topMenuAll =
      topMenu.isSuccess && topMenu.data.data.length > 0
        ? topMenu.data.data
        : [];

    var subMenus = topMenuAll.filter((menu) => {
      return menu.slug.toLowerCase().includes(category.toLocaleLowerCase());
    });
  ...
  return (
    <Fragment>
      ...
      <div className="main-content">
        ...
        <div
          className={`overlay ${isSideBarOpen ? "overlay-bg" : ""}`}
          onClick={openSideBar}
        ></div>
      </div>
    </Fragment>
